...
public:
	...
	{
	while (time < tmax_) {
		vslNewStream( &stream, VSL_BRNG_SFMT19937, 777 + rank_ + omp_get_thread_num() );
		int prev_rank = rank_ - 1;
		int next_rank = rank_ + 1;

		#pragma omp for
        	for (size_type i =  (rank_ == 0) ; i < local_N_ + 2 - (rank_ == procs_ - 1); ++i) {
		...
		if (prev_rank >= 0) {
			MPI_Irecv(&particles_[ 0], N_, MPI_VALUE_TYPE, prev_rank, 100, MPI_COMM_WORLD, &req[0]);
			MPI_Isend(&particles_[ 0], N_, MPI_VALUE_TYPE, prev_rank, 100, MPI_COMM_WORLD, &req[1]);
		} else {
		...
		if (next_rank < procs_) {
			MPI_Irecv(&particles_[(local_N_+1)*N_], N_, MPI_VALUE_TYPE, next_rank, 100, MPI_COMM_WORLD, &req[2]);
			MPI_Isend(&particles_[(local_N_+1)*N_], N_, MPI_VALUE_TYPE, next_rank, 100, MPI_COMM_WORLD, &req[3]);
		} else {
		...
		if (next_rank < procs_) {
	    		for (size_type j = 1; j < N_ - 1; ++j) {
				particles_[local_N_*N_+j] += particles_[(local_N_+1)*N_+j];	
                		particles_[(local_N_+1)*N_+j] = 0;
	    		}
		}
...
private:
    ...
    std::vector <size_type> particles_, moves_;
};
