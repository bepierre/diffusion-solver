...
public:
	...
	// 4 at a time so that the compiler can vectorize
	    ...
	    // avx loading
	    __m128i vindex = _mm_set_epi32(N_*(i+3), N_*(i+2), N_*(i+1), N_*(i));
            __m256d current = _mm256_i32gather_pd(rho_tmp.data()+1, vindex, 8);
            __m256d under   = _mm256_i32gather_pd(rho_tmp.data()+2, vindex, 8);
            __m256d above;
	    __m256d after;
            __m256d denom;
            __m256d c_p_vec;
	     ...
	     _mm256_storeu_pd(d_p_.data(), d_p_tmp);
	    	...
	    	// avx loading
                current = _mm256_i32gather_pd(rho_tmp.data()+1+j, vindex, 8);
            	under   = _mm256_i32gather_pd(rho_tmp.data()+2+j, vindex, 8);
                above   = _mm256_i32gather_pd(rho_tmp.data()+j  , vindex, 8);
                denom = _mm256_set1_pd(denom_[j-1]);
                ...
                _mm256_storeu_pd(d_p_.data()+4*j, d_p_tmp);
            }
            // Last element
	    /*
	    // avx loading
            current = _mm256_i32gather_pd(rho_tmp.data()+N_-2, vindex, 8);
            under   = _mm256_i32gather_pd(rho_tmp.data()+N_-1, vindex, 8);
            above   = _mm256_i32gather_pd(rho_tmp.data()+N_-3, vindex, 8);
	    denom = _mm256_set1_pd(denom_[N_-5]);

            _mm256_scatter_pd(rho_tmp.data()+i*N_+(N_-2), rho_tmp.data()+(i+1)*N_+(N_-2), rho_tmp.data()+(i+2)*N_+(N_-2), rho_tmp.data()+(i+3)*N_+(N_-2), _mm256_mul_pd(_mm256_fmadd_pd(factor_m_a, d_p_tmp, _mm256_fmadd_pd(factor_4,above, _mm256_mul_pd(factor_3, current))), denom));
	    */
